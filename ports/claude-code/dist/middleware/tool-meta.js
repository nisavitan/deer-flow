 ...
 */
export const PAGE_CONTENT_TOOL_NAMES = new Set(['web_fetch']);
/** `_ERROR_SHELL_PHRASES` — RFC 9110 reason phrases mapped onto the `_ERROR_RULES` category. */
export const ERROR_SHELL_PHRASES = {
    unauthorized: 'auth',
...
export const STATUS_TITLE_FILLER = new Set([
    'http',
    'error',
    'page',
    'the',
...
export const SEMANTIC_ZERO_ERROR_STRINGS = new Set([
    'none',
    'null',
    'false',
    'no',
...
const ATTRS_BY_ERROR_TYPE = Object.fromEntries(ERROR_RULES.map(([, attributes]) => [attributes.error_type, attributes]));
 ...
 */
function matchKeyword(keyword, lowered) {
    if (/^\d+$/.test(keyword))
        return new RegExp(`\\b${keyword}\\b`).test(lowered);
...
export function classifyErrorText(text) {
    ...
    for (const [keywords, attributes] of ERROR_RULES) {
        if (keywords.some((keyword) => matchKeyword(keyword, lowered)))
            return attributes;
    }
 ...
 */
export function extractJsonErrorText(content) {
    const data = parseJson(content);
    if (data === undefined)
        return null;
    const error = isJsonObject(data) ? data['error'] : null;
    // Python truthiness: None, "", 0, False, [], {} are all "no error".
    if (error === null || error === undefined || error === false || error === 0 || error === '')
        return null;
    if (Array.isArray(error) && error.length === 0)
        return null;
    if (isJsonObject(error) && Object.keys(error).length === 0)
        return null;
 ...
 */
export function asStatusLine(title) {
    let words = title
        .toLowerCase()
        .replace(/[^0-9a-z]+/g, ' ')
        .split(' ')
    ...
    while (words.length > 0) {
        ...
        if (!STATUS_TITLE_FILLER.has(head) && !isStatusCode)
            break;
        words = words.slice(1);
    }
 ...
 */
export function classifyErrorShell(toolName, content) {
    if (!PAGE_CONTENT_TOOL_NAMES.has(toolName))
        return null;
...
/** `_make_meta`. */
function makeMeta(status, source, attributes = {}) {
    return {
        status,
...
export function normalizeToolResult(input) {
    if (input.preExistingMeta !== null && input.preExistingMeta !== undefined)
        return input.preExistingMeta;
    const content = typeof input.content === 'string' ? input.content : '';
    const contentLower = content.toLowerCase();
    if (input.status === 'error' && !content.startsWith(ERROR_PREFIX)) {
        const jsonError = extractJsonErrorText(content);
        if (jsonError !== null)
            return makeMeta('error', 'tool_return', classifyErrorText(jsonError));
    ...
    if (shellAttributes !== null)
        return makeMeta('error', 'content_analysis', shellAttributes);
 ...
 */
export function stampExceptionMeta(excInfo) {
...
export function isProblemMeta(meta) {
