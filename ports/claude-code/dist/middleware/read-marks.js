 ...
 */
export function blockMessage(toolName, path, readToolName = ORIGINAL_READ_TOOL_NAME) {
    return (`Error: ${toolName} blocked — ${path} already exists and you have not read its current version. ` +
...
export function normalizeMarkPath(path) {
 ...
 */
export function hashFileIfReadable(filePath) {
    try {
        const content = readFileSync(filePath, 'utf8');
        if (content.startsWith(UNINSPECTABLE_CONTENT_PREFIX))
            return null;
...
export function parseReadMarks(value) {
 ...
 */
export function latestMarkHash(marks, normalizedPath) {
    for (let index = marks.length - 1; index >= 0; index -= 1) {
        const mark = marks[index];
        if (mark !== undefined && mark.path === normalizedPath)
            return mark.hash;
    }
    return null;
}
 ...
 */
export function stampReadMark(marks, mark) {
 ...
 */
export function checkWriteGate(input) {
    if (input.currentHash === null)
        return { allowed: true };
    const normalized = normalizeMarkPath(input.path);
    if (latestMarkHash(input.marks, normalized) === input.currentHash)
        return { allowed: true };
...
export function readMarksPath(threadId, env) {
 ...
 */
export function loadReadMarks(filePath) {
    try {
        const envelope = readStateFile(filePath);
...
export function applyReadMark(filePath, mark, options) {
    return updateStateFile(filePath, (current) => ({ marks: stampReadMark(parseReadMarks(current), mark) }), options);
}
