 ...
 */
export function extractTranscriptTail(raw, maxObjectives = DIGEST_MAX_OBJECTIVES) {
    const objectives = [];
    let messageCount = 0;
...
function readChannel(dir, fileName) {
    try {
        return readStateFile(join(dir, fileName))?.payload ?? null;
    }
    catch {
        return null; // Corrupt or unknown-schema channel: absent, never fatal inside a hook.
    }
}
function openTodos(payload) {
    ...
    for (const todo of todos) {
        ...
        if (open.length >= DIGEST_MAX_TODOS)
            break;
    }
    return open;
}
function delegationCounts(payload) {
...
function artifactPaths(payload) {
    const artifacts = payload?.artifacts;
    if (!Array.isArray(artifacts))
        return [];
    return artifacts.filter((item) => typeof item === 'string').slice(0, DIGEST_MAX_ARTIFACTS);
}
/** Assemble the digest. Pure apart from reading the named files; never throws. */
export function buildSummaryDigest(options) {
    ...
    return {
        ...
        open_todos: openTodos(readChannel(options.stateDir, TODOS_FILE)),
        delegations: delegationCounts(readChannel(options.stateDir, DELEGATIONS_FILE)),
        artifacts: artifactPaths(readChannel(options.stateDir, ARTIFACTS_FILE)),
...
export function renderDigestText(digest) {
    ...
    if (digest.open_todos.length > 0) {
        ...
        for (const todo of digest.open_todos)
    ...
    lines.push('', 'This digest lists durable state only; it does not reproduce the compacted conversation.');
    return lines.join('\n');
}
