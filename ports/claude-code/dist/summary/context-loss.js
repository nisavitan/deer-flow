...
export const RECALL_ITEM_KINDS = ['fact', 'decision', 'file'];
...
export class InvalidContextFixtureError extends Error {
    name = 'InvalidContextFixtureError';
}
const KIND_BY_FIELD = {
    facts: 'fact',
    decisions: 'decision',
...
export function matchTargets(item) {
    ...
    if (item.kind === 'file')
        add(basename(item.text));
    return [...targets];
}
/** True when the probe text contains any surface form of the item. */
export function isRecalled(item, normalizedProbe) {
...
export function parseContextSnapshot(raw) {
    ...
    for (const [field, kind] of Object.entries(KIND_BY_FIELD)) {
        ...
        if (!Array.isArray(list))
            throw new InvalidContextFixtureError(`${field} must be an array`);
        for (const [index, entry] of list.entries()) {
            ...
            if (seen.has(id))
                throw new InvalidContextFixtureError(`duplicate item id ${JSON.stringify(id)}`);
            seen.add(id);
            ...
            const aliases = (rawAliases ?? []).map((alias, aliasIndex) => assertString(alias, `${field}[${index}].aliases[${aliasIndex}]`));
...
export function parseProbeTranscript(raw) {
    const list = Array.isArray(raw) ? raw : raw?.['answers'];
    if (!Array.isArray(list))
        throw new InvalidContextFixtureError('probe must be an array or {answers: []}');
    return {
 ...
 */
export function scoreRecall(snapshot, probe) {
