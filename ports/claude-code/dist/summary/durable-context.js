...
export function escapeContextText(value) {
...
function fitsBudget(lines, candidate, maxChars) {
 ...
 */
export function renderDelegationLedger(entries, maxChars = LEDGER_RENDER_CHAR_BUDGET) {
    if (entries.length === 0)
        return '';
    const lines = [
...
export function renderSkillContext(entries) {
    if (entries.length === 0)
        return '';
    ...
    for (const entry of entries) {
        ...
        const collapsed = typeof rawDescription === 'string'
 ...
 */
export function renderActiveGoal(goal) {
...
export function renderDurableContextData(state, options = {}) {
    const dataParts = [];
 ...
 */
export function buildDurableContextBlock(state, options = {}) {
    const dataBlock = renderDurableContextData(state, options);
    if (!dataBlock)
        return { authorityContract: '', dataBlock: '', text: '' };
    return {
