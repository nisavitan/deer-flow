...
import { DEFAULT_MAX_GOAL_CONTINUATIONS, InvalidGoalObjectiveError, buildGoalState, goalPath, } from '../state/goal.js';
...
export const GOAL_CLI_COMMANDS = ['set', 'clear', 'status', 'record-continuation', 'record-evaluation'];
/**
 ...
 */
export function resolveCliThreadId(explicit, env) {
    for (const candidate of [explicit, env['DEERFLOW_THREAD_ID'], env['CLAUDE_SESSION_ID']]) {
        if (typeof candidate === 'string' && THREAD_ID_PATTERN.test(candidate))
            return candidate;
    }
    return null;
}
/** Read the current goal, or `null` when the channel is empty / unreadable. */
export function readGoal(threadId, env) {
    const envelope = readStateFile(goalPath(threadId, env));
 ...
 */
export function writeGoal(filePath, goal, now) {
    updateStateFile(filePath, () => ({ goal }), { now });
}
 ...
 */
export function runGoalCli(argv, options) {
    ...
    if (!GOAL_CLI_COMMANDS.includes(args.command)) {
    ...
    try {
        switch (command) {
            ...
            case 'set': {
                ...
                if (rawMax !== undefined) {
                    ...
                    if (!Number.isFinite(parsed))
                        return fail(`max_continuations must be a number, got ${JSON.stringify(rawMax)}`);
            ...
            case 'record-continuation': {
                ...
                if (goal === null)
                    return fail('no active goal to continue');
                const next = {
                    ...goal,
            ...
            case 'record-evaluation': {
                ...
                if (raw === undefined)
                    return fail('record-evaluation requires a JSON verdict argument');
                const goal = readGoal(threadId, env);
                if (goal === null)
                    return fail('no active goal to evaluate');
        ...
        if (error instanceof InvalidGoalObjectiveError)
