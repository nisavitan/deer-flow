...
import { GOAL_BLOCKERS } from '../state/goal.js';
 ...
 */
export const GOAL_EVALUATOR_SYSTEM_INSTRUCTION = 'You are a strict completion evaluator for an AI coding assistant.\n' +
...
export function renderGoalEvaluatorUserContent(input) {
...
export function renderGoalEvaluatorRequest(input) {
 ...
 */
export const NO_VISIBLE_EVIDENCE_EVALUATION = Object.freeze({
    satisfied: false,
 ...
 */
export function stripThinkBlocks(text, truncateUnclosed = true) {
...
export function normalizeEvaluationText(value, maxChars) {
...
export function normalizeGoalBlocker(value, satisfied) {
    if (satisfied)
        return 'none';
    if (typeof value === 'string' && GOAL_BLOCKERS.has(value) && value !== 'none')
        return value;
 ...
 */
export function parseGoalEvaluationResponse(text) {
 ...
 */
export function parseGoalVerdict(text) {
    try {
...
export function hasVisibleAssistantEvidence(messages) {
 ...
 */
export function formatVisibleConversation(messages) {
    const lines = [];
 ...
 */
export function latestVisibleAssistantSignature(messages) {
