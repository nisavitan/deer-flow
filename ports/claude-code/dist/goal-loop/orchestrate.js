...
import { attachGoalEvaluation, computeNoProgressCount, shouldContinueGoal, standDownReason, } from '../state/goal.js';
import { GOAL_EVALUATOR_SYSTEM_INSTRUCTION, NO_VISIBLE_EVIDENCE_EVALUATION, evidenceSignatureOf, renderGoalEvaluatorUserContent, } from './evaluator-prompt.js';
 ...
 */
export function makeGoalContinuationMessage(goal, evaluation) {
    const reason = evaluation.reason.length > 0 ? evaluation.reason : 'No reason provided.';
    const evidence = evaluation.evidence_summary !== undefined && evaluation.evidence_summary.length > 0
...
function resolveSignature(input) {
    if (input.evidenceSignature !== undefined)
        return input.evidenceSignature;
 ...
 */
export function decideGoalAction(input) {
 ...
 */
export const PENDING_SELF_EVALUATION = Object.freeze({
    satisfied: false,
 ...
 */
export function decideStopHook(input) {
 ...
 */
export function renderStopHookBlockReason(input) {
