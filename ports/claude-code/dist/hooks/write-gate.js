 ...
 */
export function evaluateWriteGate(payload, options = {}) {
    const env = options.env ?? process.env;
    if (env[DISABLE_READ_GATE_ENV_VAR] === '1')
        return null;
    const toolName = payload.tool_name;
    if (typeof toolName !== 'string' || !GATED_WRITE_TOOL_NAMES.has(toolName))
        return null;
    ...
    if (currentHash === null)
        return null; // Creation or uninspectable: fail open.
    let marks;
    try {
