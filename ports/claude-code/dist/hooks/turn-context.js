...
import { appendHookLog, emitHookOutput, formatCurrentDate, parseHookPayload, readStdin, resolveThreadId } from '../middleware/hook-runtime.js';
...
import { DELEGATIONS_FILE } from '../state/delegations.js';
import { GOAL_FILE } from '../state/goal.js';
import { SKILL_CONTEXT_FILE } from '../state/skill-context.js';
import { SUMMARY_FILE } from '../summary/summary-state.js';
 ...
 */
export function renderDateReminder(now) {
...
function readChannel(stateDir, fileName) {
    try {
        return readStateFile(join(stateDir, fileName))?.payload ?? null;
    }
    catch {
        return null;
    }
 ...
 */
export function buildTurnContext(payload, options) {
    const env = options.env ?? process.env;
    if (env[DISABLE_ENV_VAR] === '1')
        return null;
    const sections = [renderDateReminder(options.now)];
    const threadId = resolveThreadId(payload, env);
    if (threadId !== null) {
        try {
            const stateDir = threadStateDir(threadId, env);
            const summary = readChannel(stateDir, SUMMARY_FILE);
            const delegations = readChannel(stateDir, DELEGATIONS_FILE);
            const skillContext = readChannel(stateDir, SKILL_CONTEXT_FILE);
            const goal = readChannel(stateDir, GOAL_FILE);
...
export function evaluateTurnContext(payload, options) {
...
async function main() {
    ...
    if (output !== null)
        emitHookOutput('UserPromptSubmit', output);
}
