...
import { buildResumePlan, renderResumeReport } from './resume-plan.js';
...
export function collectReports(options) {
    return options.threadIds.map((threadId) => buildResumePlan({
        threadId,
...
export function renderStatusDocument(reports, options) {
    if (reports.length === 0) {
        return [
            '# DeerFlow status',
            '',
    ...
    for (const report of reports)
        lines.push(renderResumeReport(report));
    lines.push('---', '', '**Action key:** `continue` — state is bound to this commit, resume where it stopped. ', '`restart_stale` — HEAD moved (or cannot be proven), so cached delegation results are not ', 'reused and their stages re-run. `nothing_to_resume` — no live run, no goal, no open todos.', '');
    return lines.join('\n');
}
function main() {
    let options;
