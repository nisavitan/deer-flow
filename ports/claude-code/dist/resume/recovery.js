...
import { RUN_META_FILE, applyRunTransition, isTerminalRunStatus, } from '../state/run-meta.js';
 ...
 */
export function listStateThreads(stateRoot) {
    let entries;
    try {
...
export function readRunMeta(filePath) {
    const gate = checkSchemaGate(filePath);
    if (gate.status !== 'ok')
        return null;
 ...
 */
export function classifyOrphan(run, now, expiryMs) {
 ...
 */
export function scanOrphanRuns(options) {
    ...
    for (const threadId of listStateThreads(options.stateRoot)) {
        ...
        if (run === null)
            continue;
        if (typeof run.status !== 'string' || isTerminalRunStatus(run.status))
            continue;
        if (currentSessionId !== null && run.session_id === currentSessionId)
            continue;
 ...
 */
export function applyRecovery(orphan, options) {
    if (orphan.action !== 'mark_interrupted')
        return null;
    const envelope = applyRunTransition(orphan.filePath, {
        status: 'interrupted',
 ...
 */
export function recoverOrphanRuns(options) {
