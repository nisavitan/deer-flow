...
import { SUBAGENT_STOP_REASON_VALUES } from '../policy/stop-reason.js';
/**
 ...
 */
export const TASK_RESULT_STATUS_VALUES = ['completed', 'failed', 'timed_out', 'cancelled'];
/**
...
export const DEEP_RUN_TASK_RESULT_SCHEMA = {
    ...
    properties: {
        status: {
            ...
            description: 'Terminal outcome of this delegated task. "completed" even when a guardrail cap ended the run early but usable work survived (report the cap on stop_reason).',
        },
        result: {
            type: 'string',
            description: 'The full result text for the lead agent: summary of what was accomplished, key findings, file paths, issues. This IS the return value, not a message to a human.',
        },
        stop_reason: {
            type: ['string', 'null'],
            enum: [...TASK_RESULT_STOP_REASON_VALUES, null],
            description: 'Why a guardrail cap ended the run early, or null when none did. token_capped = ran out of token budget; turn_capped = ran out of turns; loop_capped = repeated the same tool call.',
        },
        files_produced: {
            type: 'array',
            items: { type: 'string' },
...
export const DEEP_RUN_AGENT_TYPES = ['deerflow-general-purpose', 'deerflow-bash'];
 ...
 */
export function coerceWorkflowArgs(value) {
    let candidate = value;
...
export function isTaskResultStatus(value) {
    return typeof value === 'string' && TASK_RESULT_STATUS_VALUES.includes(value);
}
export function isTaskResultStopReason(value) {
    return typeof value === 'string' && TASK_RESULT_STOP_REASON_VALUES.includes(value);
}
/**
 ...
 */
export function normalizeTaskResult(raw, sentinel) {
    ...
    const files = Array.isArray(source['files_produced'])
        ? source['files_produced'].filter((entry) => typeof entry === 'string')
        : [];
    return {
        status,
