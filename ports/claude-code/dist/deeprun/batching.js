...
import { SUBAGENT_LIMIT_NOTE, SUBAGENT_LIMIT_STOP_REASON, allowedThisResponse, createSubagentLimits, } from '../policy/caps.js';
/**
 ...
 */
export function resolveAgentType(subagentType) {
    const bare = typeof subagentType === 'string' ? subagentType.replace(/^deerflow:/, '') : subagentType;
    if (bare === 'deerflow-bash' || bare === 'bash')
        return 'deerflow-bash';
 ...
 */
export function qualifyAgentType(agentType) {
 ...
 */
export function planBatches(options) {
    ...
    while (cursor < tasks.length) {
        ...
        if (decision.allowedTaskCalls === 0)
            break;
        const batch = [];
