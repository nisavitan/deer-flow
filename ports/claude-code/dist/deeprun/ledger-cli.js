...
import { delegationsPath } from '../state/delegations.js';
...
const RUN_STOP_REASONS = new Set([
    'loop_capped',
    'token_capped',
    'safety_capped',
 ...
 */
export function buildLedgerEntries(result, now) {
    const problems = [];
    ...
    raw.forEach((item, position) => {
        ...
        if (id === null)
            problems.push(`entry ${position} has no \`id\``);
        if (typeof description !== 'string')
            problems.push(`entry ${position} has no \`description\``);
        if (subagentType === null)
            problems.push(`entry ${position} has no \`subagent_type\``);
        if (status === null)
            problems.push(`entry ${position} has no \`status\``);
        if (id === null || typeof description !== 'string' || subagentType === null || status === null)
            return;
        const entry = {
            id,
            description,
        ...
        if (brief !== null)
            entry.result_brief = boundMetadataText(brief);
    ...
    if (problems.length > 0)
        throw new LedgerValidationError(problems);
    return entries;
}
/**
 ...
 */
export function persistDeepRunLedger(result, options) {
    ...
    if (stopReason !== null && RUN_STOP_REASONS.has(stopReason)) {
        try {
            ...
            const existing = readStateFile(filePath)?.payload.run ?? null;
            const runId = asString(result.run_id);
            if (existing !== null && (runId === null || existing.run_id === runId)) {
                applyRunTransition(filePath, { status: existing.status, stopReason: stopReason, now: options.now }, { now: options.now });
...
export function parseArgs(argv, env = process.env) {
    const options = { threadId: env['DEERFLOW_THREAD_ID'] ?? null, json: false };
...
function main() {
    ...
    let result;
    try {
        const parsed = JSON.parse(readStdinSync());
    ...
    try {
        ...
        if (options.json) {
            ...
            process.stdout.write(`deerflow ledger: ${outcome.entries} delegation(s) persisted to ${outcome.path} ` +
