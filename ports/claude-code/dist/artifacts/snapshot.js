...
import { closeSync, lstatSync, openSync, readSync, readdirSync, readlinkSync, statSync } from 'node:fs';
import { createHash } from 'node:crypto';
import { join, relative, sep } from 'node:path';
import { isSecretLikeName } from '../policy/env-scrub.js';
import { readStateFile, threadStateFile, writeStateFile } from '../state/index.js';
 ...
 * report a change.
 */
export const EXCLUDED_DIR_NAMES = Object.freeze(new Set([
    '.git',
    '.hg',
    '.svn',
    '.cache',
...
export function isSensitiveWorkspacePath(path) {
    for (const segment of path.split('/')) {
        if (segment.length === 0)
            continue;
        if (isSecretLikeName(segment))
            return true;
        if (EXTRA_SENSITIVE_PATTERNS.some((pattern) => pattern.test(segment)))
            return true;
    }
    return false;
}
 ...
 */
export function defaultScanRoots(projectRoot, workspaceMaxDepth = FAST_WORKSPACE_MAX_DEPTH) {
    return [
        {
            name: 'workspace',
            dir: projectRoot,
...
function relativePosix(projectRoot, absolute) {
    return relative(projectRoot, absolute).split(sep).join('/');
}
function snapshotSymlink(projectRoot, absolute, root) {
...
function snapshotFile(projectRoot, absolute, root, limits) {
    let size;
    let mtimeMs;
    try {
    ...
    const reason = binary ? 'binary' : oversized ? 'large' : null;
    return {
        path,
        root,
        size,
 ...
 */
export function scanWorkspace(roots, projectRoot, options = {}) {
            ...
            for (const entry of sorted) {
                ...
                if (entry.isDirectory()) {
                    if (EXCLUDED_DIR_NAMES.has(entry.name))
                        continue;
                    if (root.prune?.has(entry.name) === true && current.depth === 0)
                        continue;
                    if (root.maxDepth !== null && current.depth + 1 > root.maxDepth)
                        continue;
...
function toChange(status, before, after) {
    const sample = after ?? before;
    /* c8 ignore next */
    if (sample === null)
        throw new Error('unreachable: a change needs at least one side');
    return {
        path: sample.path,
        root: sample.root,
        status,
 ...
 */
export function diffSnapshots(before, after, limits = DEFAULT_WORKSPACE_LIMITS) {
    const created = [];
    const modified = [];
    const deleted = [];
    ...
    for (const path of paths) {
        ...
        if (beforeFile !== null && afterFile !== null && sameFile(beforeFile, afterFile))
            continue;
        ...
        recorded += 1;
        if (beforeFile === null)
            created.push(toChange('created', null, afterFile));
        else if (afterFile === null)
            deleted.push(toChange('deleted', beforeFile, null));
        else
            modified.push(toChange('modified', beforeFile, afterFile));
    }
 ...
 */
export function changedOutputPaths(diff) {
...
export function workspacePrePath(threadId, env) {
...
export function writePreSnapshot(filePath, snapshot, now) {
    ...
    return writeStateFile(filePath, { snapshot }, { now });
}
/**
 ...
 */
export function readPreSnapshot(filePath) {
    let payload;
    try {
        payload = readStateFile(filePath)?.payload ?? null;
    }
    catch {
