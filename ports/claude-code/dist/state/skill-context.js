...
export function normalizeSkillEntry(entry) {
    ...
    if (path === undefined || path === null)
        throw new MissingSkillPathError();
 ...
 */
export function mergeSkillContext(existing, incoming) {
...
export function skillContextPath(threadId, env) {
    return threadStateFile(threadId, SKILL_CONTEXT_FILE, env);
}
/** Apply a skill-context write under atomic-write + `rev` CAS. */
export function applySkillContext(filePath, incoming, options) {
    return updateStateFile(filePath, (current) => ({ entries: mergeSkillContext(current?.entries ?? null, incoming) }), options);
}
