...
<!-- deerflow-origin: ported from skills/public/ppt-generation/ @ 0950924
     port: script/workspace/output paths; present_files -> final-message listing; capability gate prepended. Confirmed provider-dependent: every slide is rendered through image-generation/scripts/generate.py.
-->
...
Required: the **image-generation optional module installed alongside this one** (every
slide is rendered by `image-generation/scripts/generate.py`), an image-provider API key
- `GEMINI_API_KEY` or `MINIMAX_API_KEY` - and Python 3 with `python-pptx` for
`scripts/generate.py`.

```bash
test -n "$GEMINI_API_KEY" || test -n "$MINIMAX_API_KEY" \
  || echo "MISSING: no image provider key (GEMINI_API_KEY or MINIMAX_API_KEY)"
# run this from the directory that contains this SKILL.md:
ls ../image-generation/scripts/generate.py 2>/dev/null \
  || echo "MISSING: image-generation optional module is not installed next to this skill"
python3 -c 'import pptx' 2>/dev/null || echo "MISSING: python-pptx"
```
...
3. how to enable this module if it is not installed yet - copy
   `ports/claude-code/skills-optional/ppt-generation/` into the project's `.claude/skills/ppt-generation/`
   (or into `~/.claude/skills/ppt-generation/` for personal scope), or enable it through your
   Claude Code settings, then restart the session.

Then end your turn. Nothing below this section runs until the check passes.

When the check passes, set the path variables used by the commands below:

```bash
SKILL_DIR=<absolute path of the directory containing this SKILL.md>
IMAGE_SKILL_DIR=<absolute path of the installed image-generation module>
```
...
1. Read the image-generation skill: `$IMAGE_SKILL_DIR/SKILL.md`
...
Read `$IMAGE_SKILL_DIR/SKILL.md` to understand how to generate images.
...
```bash
python "$SKILL_DIR"/scripts/generate.py \
  --plan-file workspace/nova-plan.json \
  ...
  --output-file outputs/nova-presentation.pptx
```
...
- The PPTX file is saved in `outputs/`
- Share the generated presentation with the user by listing its path in your final message
