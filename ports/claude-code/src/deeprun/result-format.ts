...
import {
  ...
  assertSubagentStatus,
  assertSubagentStopReason,
  stopReasonLabel,
  type SubagentStatus,
 ...
 * the rendered length.
 */
export function boundMetadataText(text: string, cap: number = SUBAGENT_METADATA_TEXT_MAX_CHARS): string {
  const cleaned = text.trim()
  const points = [...cleaned]
  if (points.length <= cap) return cleaned
  const marker = '\n...\n'
  if (cap <= marker.length) return points.slice(0, cap).join('')
  const head = Math.floor((cap * 2) / 3)
  const tail = cap - head - marker.length
  if (tail <= 0) return points.slice(0, cap).join('')
...
export interface SubagentResultInput {
  ...
  readonly stopReason?: SubagentStopReason | null
}

export interface FormattedSubagentResult {
  /** Text the lead model sees in place of the tool result. */
  readonly modelVisibleContent: string
  /** Normalized error the metadata channel records, or `null` for a success. */
...
export function normalizeTokenUsage(value: unknown): TokenUsage | null {
  ...
  const normalized: Partial<TokenUsage> = {}
  ...
  return normalized as TokenUsage
}

export interface AdditionalKwargsInput extends SubagentResultInput {
  readonly modelName?: string | null
  readonly tokenUsage?: unknown
}

export type SubagentAdditionalKwargs = Record<string, unknown>
...
export function makeSubagentAdditionalKwargs(
  status: SubagentStatus,
  input: AdditionalKwargsInput = {},
): SubagentAdditionalKwargs {
  assertSubagentStatus(status)
  const stopReason = assertSubagentStopReason(input.stopReason ?? null)

  const payload: SubagentAdditionalKwargs = { [SUBAGENT_STATUS_KEY]: status }
  ...
  if (stopReason !== null) payload[SUBAGENT_STOP_REASON_KEY] = stopReason
  ...
  if (usage !== null) payload[SUBAGENT_TOKEN_USAGE_KEY] = usage
