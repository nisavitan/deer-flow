 ...
 */
export const NEXT_ACTION_GUIDANCE: Readonly<Record<RecommendedNextAction, string>> = {
...
export function evaluateToolResult(payload: HookPayload, options: ClassifyOptions = {}): HookOutput | null {
  ...
  if (isProblemMeta(meta)) {
    lines.push(
      `<${TOOL_META_KEY} tool="${toolName}">`,
      JSON.stringify(meta),
      `</${TOOL_META_KEY}>`,
      `Recommended next action: ${meta.recommended_next_action} — ${NEXT_ACTION_GUIDANCE[meta.recommended_next_action]}`,
    )
