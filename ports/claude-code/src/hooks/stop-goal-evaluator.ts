...
import {
  ...
  type VisibleMessage,
} from '../goal-loop/evaluator-prompt.js'
import { decideStopHook, type StopHookDecision } from '../goal-loop/orchestrate.js'
 ...
 */
export function parseTranscriptMessages(raw: string): VisibleMessage[] {
...
export function readTranscript(transcriptPath: string | null): VisibleMessage[] {
...
export interface EvaluateStopOptions {
  readonly now: string
  readonly env?: NodeJS.ProcessEnv
}

export interface StopHookOutcome {
  readonly decision: StopHookDecision | null
  /** The thread whose goal was consulted, or `null` when the hook stood down before reading. */
  readonly threadId: string | null
  /** Whether `nextGoal` was persisted. */
 ...
 */
export function evaluateStop(payload: StopHookPayload, options: EvaluateStopOptions): StopHookOutcome {
  const env = options.env ?? process.env
  const threadId = resolveThreadId(payload, env)
  if (threadId === null) return { decision: null, threadId: null, persisted: false }
  ...
  try {
    ...
    process.stderr.write(
      `deerflow stop-goal-evaluator: standing down (${error instanceof Error ? error.message : String(error)})\n`,
    )
...
export function renderHookOutput(outcome: StopHookOutcome): string {
  const decision = outcome.decision
  if (decision === null || !decision.block || decision.blockReason === null) return ''
...
async function main(): Promise<void> {
  ...
  appendHookLog({
    ...
    summary:
      `continuations=${outcome.decision?.continuationCount ?? 0} no_progress=${outcome.decision?.noProgressCount ?? 0} ` +
