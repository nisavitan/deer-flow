...
import { applyRunTransition, readStateFile, type DeliveryReceipt, type RunMetaPayload } from '../state/index.js'
 ...
 */
export const OUTPUTS_DELIVERY_POLICY =
  'Files produced for the user land in ./outputs/, and every file placed there MUST be listed in the final response.'

/** One delivery verdict: what was produced, what was presented, and what is missing. */
export interface DeliveryVerdict {
  /** True when every produced path is covered by a presented path. */
  readonly satisfied: boolean
  /** Created/modified regular files under `outputs/` this turn (`get_changed_output_paths`). */
  readonly produced_paths: string[]
  /** Path-like tokens found in the final assistant message, deduplicated and sorted. */
  readonly presented_paths: string[]
  /** The produced paths a presented token covers — the intersection the original checked. */
  readonly matched_paths: string[]
  /** Produced paths with no covering mention. Non-empty ⇒ `satisfied === false`. */
 ...
 */
export function extractPresentedPaths(text: string): string[] {
  ...
  for (const raw of text.match(PATH_TOKEN_RE) ?? []) {
    ...
    if (normalized.length === 0 || normalized === '/' || normalized === '.') continue
 ...
 */
export function coversProducedPath(presented: string, produced: string): boolean {
  if (presented === produced) return true
  if (produced.endsWith(`/${presented}`)) return true
  if (presented.endsWith(`/${produced}`)) return true
  return false
}

export interface DeliveryInput {
  /** Created/modified regular files under `outputs/`; see `changedOutputPaths` in ./snapshot.ts. */
 ...
 * the user nothing.
 */
export function evaluateDelivery(input: DeliveryInput): DeliveryVerdict {
...
/**
 * The receipt written into `run-meta.json`.
 *
 * `presented_paths` keeps the meaning `src/state/run-meta.ts` gave it — the paths credited as
 * delivered — so the recovery backfill's zero receipt (`presented_paths: []`) stays comparable.
 * The token list the extractor produced is NOT what lands there; `matched_paths` and
 * `presented_paths` are the same set for that reason, the former named for the original's verdict
 * vocabulary and the latter for the existing field contract.
 */
export interface DeliveryReceiptFields extends DeliveryReceipt {
  readonly produced_paths: string[]
  readonly matched_paths: string[]
  readonly satisfied: boolean
}
...
export function buildDeliveryReceipt(verdict: DeliveryVerdict, now: string): DeliveryReceiptFields {
  return {
...
/** What {@link recordDeliveryReceipt} did. */
export type ReceiptOutcome =
  /** No run record exists for this thread; nothing was written. */
  | { readonly kind: 'no-run' }
  /** The receipt was written into the existing run record. */
  | { readonly kind: 'written'; readonly receipt: DeliveryReceiptFields }
  /** A receipt was already present and was preserved (put-if-absent). */
  | { readonly kind: 'preserved'; readonly receipt: DeliveryReceipt }
...
export function recordDeliveryReceipt(
  ...
  const current = readStateFile<RunMetaPayload>(filePath)?.payload.run ?? null
  if (current === null) return { kind: 'no-run' }
  if (current.delivery !== undefined) return { kind: 'preserved', receipt: current.delivery }
 ...
 */
export function renderDeliveryBlockReason(verdict: DeliveryVerdict): string {
...
export function extractFinalAssistantText(raw: string): string {
  ...
  for (const line of raw.split('\n')) {
    ...
    const content =
      typeof message === 'object' && message !== null ? (message as Record<string, unknown>)['content'] : record['content']
