...
import { updateStateFile, type StateEnvelope, type UpdateOptions } from '../state/atomic-io.js'
import type { WorkspaceChangeSummary, WorkspaceDiff } from './snapshot.js'
...
/** One turn's record. The port's analogue of a single `workspace_changes` event. */
export interface WorkspaceChangesEntry {
  readonly recorded_at: string
  /** Human-readable one-liner, the original's event `content` minus the +A/-D line counters. */
  readonly content: string
  readonly summary: WorkspaceChangeSummary
  readonly created: string[]
  readonly modified: string[]
  readonly deleted: string[]
  /** Created/modified regular files under `outputs/` — the delivery verdict's input. */
  readonly outputs_changed: string[]
  /** True when any of the three path lists was cut at {@link MAX_PATHS_PER_ENTRY}. */
  readonly paths_truncated: boolean
}

export interface WorkspaceChangesPayload {
  /** Oldest first, newest last, at most {@link MAX_HISTORY_ENTRIES}. */
  entries: WorkspaceChangesEntry[]
  [key: string]: unknown
}

function capPaths(paths: readonly string[]): { paths: string[]; truncated: boolean } {
  if (paths.length <= MAX_PATHS_PER_ENTRY) return { paths: [...paths], truncated: false }
 ...
 */
export function renderChangeContent(summary: WorkspaceChangeSummary): string {
...
export function buildChangesEntry(diff: WorkspaceDiff, now: string): WorkspaceChangesEntry {
 ...
 */
export function describesSameDelta(a: WorkspaceChangesEntry, b: WorkspaceChangesEntry): boolean {
  return (
    samePathList(a.created, b.created) &&
 ...
 */
export function appendChangesEntry(
  existing: readonly WorkspaceChangesEntry[] | null | undefined,
  entry: WorkspaceChangesEntry,
): WorkspaceChangesEntry[] {
  const current = existing ?? []
  const latest = current.length > 0 ? current[current.length - 1] : undefined
  if (latest !== undefined && describesSameDelta(latest, entry)) return [...current]
...
export function workspaceChangesPath(threadId: string, env?: NodeJS.ProcessEnv): string {
...
): StateEnvelope<WorkspaceChangesPayload> {
  return updateStateFile<WorkspaceChangesPayload>(
    filePath,
    (current) => ({
...
export function latestChangesEntry(payload: WorkspaceChangesPayload | null): WorkspaceChangesEntry | null {
  if (payload === null || !Array.isArray(payload.entries) || payload.entries.length === 0) return null
