...
export interface GitHead {
  ...
  /** `null` on a detached HEAD — there is no branch to compare. */
  readonly branch: string | null
}

/** `git rev-parse --abbrev-ref HEAD` prints the literal `HEAD` when detached. */
export function interpretBranch(raw: string | null): string | null {
...
export function readGitHead(cwd: string): GitHead {
  return {
