...
import { GOAL_FILE, type GoalPayload, type GoalState } from '../state/goal.js'
import { RUN_META_FILE, isTerminalRunStatus, type RunMeta, type RunMetaPayload } from '../state/run-meta.js'
import { TODOS_FILE, type TodosPayload } from '../state/todos.js'
import { checkSchemaGate, evaluateStaleness, partitionByCommitBinding, type StalenessReport } from './staleness.js'
...
/** What the caller should do with this thread. */
export type RecommendedAction = 'continue' | 'restart_stale' | 'nothing_to_resume'

export interface ResumeTodo {
  readonly content: string
  readonly status: string
}

export interface ResumeDelegation {
  readonly id: string
  readonly description: string
  readonly subagent_type: string
  readonly status: string
  readonly commit_sha: string | null
  /** `true` when the stage-skip predicate's commit-binding half holds for this entry. */
  readonly reusable: boolean
}

export interface ResumeDelegationSummary {
  readonly total: number
  /** Entry count per `status` value, keys sorted. */
  readonly by_status: Record<string, number>
  /** Terminal entries whose `commit_sha` matches current HEAD — skippable stages. */
  readonly reusable: number
  /** Entries excluded from the stage-skip predicate; kept in the ledger, but their stages re-run. */
  readonly invalidated: number
  /** The last {@link RESUME_MAX_DELEGATIONS} entries, oldest first (ledger order). */
  readonly recent: ResumeDelegation[]
}
...
export interface ResumeGoal {
  ...
  readonly max_no_progress_continuations: number
...
export interface ResumeReport {
  ...
  /** `true` when the recorded run still holds the thread's active-run slot. */
  readonly run_active: boolean
  readonly goal: ResumeGoal | null
  readonly open_todos: ResumeTodo[]
  /** Total open todos before the {@link RESUME_MAX_TODOS} cap. */
  readonly open_todo_count: number
  readonly delegations: ResumeDelegationSummary
  readonly artifacts: string[]
  readonly summary_text: string | null
  readonly staleness: StalenessReport
  readonly recommended_action: RecommendedAction
  /** Channel files this build could not interpret; they were read as empty, never guessed at. */
  readonly unreadable: string[]
}

export interface BuildResumePlanOptions {
  readonly threadId: string
  /** `<state root>/<thread id>`. */
  readonly stateDir: string
  /** `git rev-parse HEAD` now, or `null` when it could not be read. */
...
function readChannel<T extends Record<string, unknown>>(
  stateDir: string,
  fileName: string,
  unreadable: string[],
): T | null {
  try {
    const gate = checkSchemaGate<T>(join(stateDir, fileName))
...
function summarizeGoal(goal: GoalState | null): ResumeGoal | null {
  ...
  return {
    ...
    max_no_progress_continuations:
      typeof goal.max_no_progress_continuations === 'number' ? goal.max_no_progress_continuations : 0,
...
function summarizeDelegations(
  ...
  for (const key of Object.keys(byStatus).sort()) sorted[key] = byStatus[key] ?? 0
 ...
 */
export function buildResumePlan(options: BuildResumePlanOptions): ResumeReport {
  ...
  const runPayload = readChannel<RunMetaPayload>(stateDir, RUN_META_FILE, unreadable)
  const run = runPayload?.run ?? null
  const runActive = run !== null && typeof run.status === 'string' && !isTerminalRunStatus(run.status)

  const goalPayload = readChannel<GoalPayload>(stateDir, GOAL_FILE, unreadable)
  const goal = summarizeGoal(goalPayload?.goal ?? null)

  const todosPayload = readChannel<TodosPayload>(stateDir, TODOS_FILE, unreadable)
  const rawTodos = Array.isArray(todosPayload?.todos) ? todosPayload.todos : []
  const openTodos: ResumeTodo[] = []
  ...
  const delegationsPayload = readChannel<DelegationsPayload>(stateDir, DELEGATIONS_FILE, unreadable)
  ...
  const artifactsPayload = readChannel<ArtifactsPayload>(stateDir, ARTIFACTS_FILE, unreadable)
  const artifacts = Array.isArray(artifactsPayload?.artifacts)
    ? artifactsPayload.artifacts.filter((path): path is string => typeof path === 'string')
    : []

  const summaryPayload = readChannel<Record<string, unknown>>(stateDir, SUMMARY_FILE, unreadable)
  ...
  const recommendedAction: RecommendedAction = !hasWork
    ? 'nothing_to_resume'
    : hasStaleCache
      ? 'restart_stale'
      : 'continue'
...
export function buildResumePlans(
  threadIds: readonly string[],
  stateRoot: string,
  options: Omit<BuildResumePlanOptions, 'threadId' | 'stateDir'>,
): ResumeReport[] {
...
export function renderResumeLine(report: ResumeReport): string {
  const parts: string[] = [`thread ${report.thread_id}`]
  if (report.run !== null) {
  ...
  if (report.goal !== null) {
    parts.push(
      `goal "${bound(report.goal.objective, 80)}" ` +
        `(${report.goal.continuation_count}/${report.goal.max_continuations} continuations)`,
    )
  }
  if (report.open_todo_count > 0) parts.push(`${report.open_todo_count} open todo(s)`)
...
export function renderResumeReport(report: ResumeReport): string {
  const lines: string[] = [`## Thread \`${report.thread_id}\``, '']
  ...
  if (report.run === null) {
    ...
    lines.push(
      `- **Run:** \`${run.run_id}\` — ${run.status}` +
        ...
        `${run.error ? ` — error: ${bound(run.error, 120)}` : ''}`,
      `- **Started:** ${run.started_at} · **Updated:** ${run.updated_at}${run.ended_at ? ` · **Ended:** ${run.ended_at}` : ''}`,
      `- **Recorded commit:** ${run.commit_sha || 'none'}${run.branch ? ` (${run.branch})` : ''}`,
    )
  ...
  if (report.goal === null) {
    ...
    lines.push(
      `- **Goal:** ${report.goal.objective}`,
      `  - continuations ${report.goal.continuation_count}/${report.goal.max_continuations}` +
  ...
  if (report.open_todo_count === 0) {
    ...
    for (const todo of report.open_todos) lines.push(`  - [${todo.status}] ${todo.content}`)
  ...
  const statusPairs = Object.entries(report.delegations.by_status)
    ...
    .join(', ')
  lines.push(
    `- **Delegations:** ${report.delegations.total} total` +
  ...
  for (const entry of report.delegations.recent) {
    lines.push(
      `  - \`${entry.id}\` [${entry.status}] ${entry.subagent_type}: ${entry.description}` +
