...
export function mergePromoted(
  ...
  incoming: Partial<PromotedTools> | null | undefined,
): PromotedTools | null {
...
export function promotedPath(threadId: string, env?: NodeJS.ProcessEnv): string {
  return threadStateFile(threadId, PROMOTED_FILE, env)
}

/** Apply a promotion write under atomic-write + `rev` CAS. */
export function applyPromoted(
  filePath: string,
  incoming: Partial<PromotedTools> | null,
  options: UpdateOptions,
): StateEnvelope<PromotedPayload> {
  return updateStateFile<PromotedPayload>(
    filePath,
    (current) => ({ promoted: mergePromoted(current?.promoted ?? null, incoming) }),
    options,
  )
}
