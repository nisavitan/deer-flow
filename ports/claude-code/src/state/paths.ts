...
export const STATE_DIR_SEGMENTS = ['.deerflow', 'state'] as const
...
export class InvalidThreadIdError extends Error {
  override readonly name = 'InvalidThreadIdError'
  readonly threadId: unknown

  constructor(threadId: unknown) {
...
export function threadStateDir(threadId: string, env?: NodeJS.ProcessEnv): string {
  return join(stateRoot(env), validateThreadId(threadId))
}

/** Absolute path of one channel file inside a thread's state directory. */
export function threadStateFile(threadId: string, fileName: string, env?: NodeJS.ProcessEnv): string {
...
export function threadRunsDir(threadId: string, env?: NodeJS.ProcessEnv): string {
