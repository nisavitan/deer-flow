...
/** Golden render configurations, keyed by baseline filename stem. */
export type GoldenConfigId =
  | 'subagents_enabled_n3_two_skills'
  | 'subagents_enabled_n1'
  | 'subagents_disabled_non_interactive';

export const GOLDEN_CONFIG_IDS: readonly GoldenConfigId[] = [
  'subagents_enabled_n3_two_skills',
  'subagents_enabled_n1',
  'subagents_disabled_non_interactive',
];
...
export interface Substitution {
  ...
  readonly id: string;
  /** `literal` = exact string replace (all occurrences); `regex` = pattern replace. */
  readonly kind: 'literal' | 'regex';
  /** Literal golden text, or — for `kind: 'regex'` — the pattern source. */
  readonly original: string;
  /** Human-readable description of the pattern. Required for `kind: 'regex'`. */
  readonly originalDescription?: string;
  /** Text the port renders instead. */
  readonly replacement: string;
  /** Why the deviation is forced by the platform. */
  readonly reason: string;
  /** Configurations in which this substitution must apply. Defaults to all three. */
  readonly appliesTo?: readonly GoldenConfigId[];
}
...
const ALL: readonly GoldenConfigId[] = GOLDEN_CONFIG_IDS;

export const LEAD_PROMPT_SUBSTITUTIONS: readonly Substitution[] = [
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  {
    ...
    originalDescription:
      'The 8-line "# BASELINE PROMPT RENDER ..." comment block that extract_vectors.py prepends to each golden file.',
    replacement: '',
    reason:
      'Extractor provenance header written by ports/claude-code/parity/baseline/extract_vectors.py; it is not part of the prompt the engine returned.',
    appliesTo: ALL,
  },
...
3. If the skill matches, Read its SKILL.md at the listed location to load full instructions`,
    reason:
      'The `describe_skill` tool and its SkillCatalog query grammar are platform-native in Claude Code (map §2.5): skill names + descriptions are always in context and bodies load through the native Skill tool / Read. The surrounding four-step discovery wording is otherwise preserved verbatim.',
  ...
  },
  {
    ...
    reason:
      'The `/mnt/skills` container projection (skills.container_path) and its enabled-only projection trees do not exist in the port; skills ship inside the plugin package (map §2.4).',
    appliesTo: ['subagents_enabled_n3_two_skills'],
  },
...
\`\`\``,
    reason:
      'Tool-name remap (`read_file`→Read, `task()`→Agent delegation) plus `/mnt/user-data/workspace` → project-relative path. The fence language changes from `python` to `text` because the snippet is illustrative pseudo-code, not a callable Python API, in the port.',
  ...
  },
  {
    ...
    reason:
      'Tool-name remap of the direct-execution tool list. DeerFlow `ls` has no standalone Claude Code equivalent and folds into Glob/Bash (map §1.2 forced change 1).',
    appliesTo: ['subagents_enabled_n3_two_skills', 'subagents_enabled_n1'],
  },
  ...
  // -------------------------------------------------------------------------
  {
    ...
    reason:
      'Map §1p: the deferred-mode `skill_first_reminder` wording is dropped as platform-native (no describe_skill in Claude Code); the legacy wording is reused verbatim from prompt.py:1062.',
    appliesTo: ALL,
  },
  {
    ...
    reason:
      '`/mnt/user-data/outputs` becomes the project-relative `outputs/` contract (map §1m). The `skill_manage` carve-out is dropped: the tool does not exist and skills are edited as plain files in the plugin skills/ directory.',
    appliesTo: ALL,
  },
  {
  ...
  - Use "\`\`\`mermaid" for Mermaid diagrams.`,
    reason:
      'Virtual artifact paths (`/mnt/user-data/outputs/...`) become project-relative `outputs/` paths, so the "never use a workspace-relative filename" rule inverts and is dropped. `present_files` does not exist — there is no separate presentation step in the port (map §1m/§1p).',
    appliesTo: ALL,
  },
  ...
  // -------------------------------------------------------------------------
  {
    ...
    reason:
      'The DeerFlow web UI / Gateway is not part of the port; neutral phrasing per map §1c. The memory block itself and its confidentiality carve-out are preserved.',
    appliesTo: ALL,
  },
 ...
 * rather than silent.
 */
export const UNCOVERED_BY_GOLDENS: readonly { readonly what: string; readonly why: string }[] = [
  {
...
export function applySubstitutions(
  goldenText: string,
  configId: GoldenConfigId,
  substitutions: readonly Substitution[] = LEAD_PROMPT_SUBSTITUTIONS,
