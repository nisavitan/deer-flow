...
export interface SubagentDescriptor {
  /** Registry name, e.g. `general-purpose`. */
  readonly name: string;
  /**
   * Model-visible description. Ignored for the two built-in roles, which carry
   * the engine's own compact descriptions. Only the first line is used, and it
   * is html-escaped (the value is agent-editable in the original engine).
   */
 ...
 */
const BUILTIN_SUBAGENT_DESCRIPTIONS: Readonly<Record<string, string>> = {
  'general-purpose':
    'For bounded work with clear delegation benefit from specialist capability, context isolation, or independent parallel execution.',
...
export function buildAvailableSubagentsDescription(subagents: readonly SubagentDescriptor[]): string {
  const lines: string[] = [];
...
export interface SubagentSectionOptions {
  ...
  readonly subagents?: readonly SubagentDescriptor[];
}

/** Ported from `_build_subagent_section` (prompt.py:341-473). */
export function renderSubagentSection(options: SubagentSectionOptions = {}): string {
  ...
  let parallelDispatchGuidance: string;
  let validBenefits: string;
  let limitActionGuidance: string;
  let followupGuidance: string;
  let workflow: string;
  let examples: string;
  let multiBatchExample: string;
...
export interface SkillSectionOptions {
  ...
  readonly skillsLocation?: string;
}

/** Ported from `get_skill_index_prompt_section` (skills/describe.py:150-187). */
export function renderSkillSystemSection(options: SkillSectionOptions = {}): string {
...
export interface WorkingDirectoryOptions {
  /** Optional absolute project root. When omitted the block stays path-neutral. */
 ...
 */
export function renderWorkingDirectorySection(options: WorkingDirectoryOptions = {}): string {
  const root =
 ...
 */
export function renderClarificationSystem(nonInteractive = false): string {
...
export interface CriticalRemindersOptions {
 ...
 */
export function renderCriticalReminders(options: CriticalRemindersOptions = {}): string {
  const n = clampSubagentConcurrency(options.maxConcurrentSubagents ?? DEFAULT_MAX_CONCURRENT_SUBAGENTS);
  const total = clampTotalSubagentsPerRun(options.maxTotalSubagents ?? DEFAULT_MAX_TOTAL_SUBAGENTS_PER_RUN);
  const reminderBenefits =
    n === 1 ? 'specialist capability or context isolation' : 'real parallel latency, specialist capability, or context isolation';
...
export interface LeadPromptOptions {
  /** `<role>` name. Defaults to `DeerFlow 2.0` (prompt.py:1072). */
  readonly agentName?: string;
  /**
   * Optional model info line. Port-only addition (no DeerFlow equivalent) and
   * therefore absent from every golden render; omitted by default.
   */
  readonly modelInfo?: string;
  /** Optional identity text (SOUL.md equivalent). Rendered html-escaped inside `<soul>`. */
  readonly soul?: string;
  /** Optional absolute project root for the `<working_directory>` block. */
  readonly workingDirectory?: string;
  readonly skillNames?: readonly string[];
  readonly skillsLocation?: string;
  readonly subagentEnabled?: boolean;
  ...
  readonly maxTotalSubagents?: number;
  readonly subagents?: readonly SubagentDescriptor[];
  /** Scheduler-style run with no human available (DeerFlow `context.non_interactive`). */
  readonly nonInteractive?: boolean;
  /** DeerFlow `configurable.is_plan_mode`: appends the todo-list discipline block. */
  readonly planMode?: boolean;
}

/** Ported from `get_agent_soul` (prompt.py:880-891). */
function renderSoul(soul: string | undefined): string {
...
function renderModelInfo(modelInfo: string | undefined): string {
 ...
 */
export function renderLeadPrompt(options: LeadPromptOptions = {}): string {
  ...
  const workingDirectorySection = renderWorkingDirectorySection(
