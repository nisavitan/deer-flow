...
import { CORE_CATEGORIES, normalizeCategory, type FactCategory } from './store.js'
...
/** Extraction-only labels. Evaluated by this gate, never persisted. */
export type FactScope = 'user' | 'thread' | 'project'
export type FactDurability = 'durable' | 'temporary'
export type FactAuthority = 'descriptive' | 'transactional'

/** One proposed fact as the extraction prompt emits it (`newFacts[]`). */
export interface ProposedFact {
  readonly content?: unknown
  readonly category?: unknown
  readonly confidence?: unknown
  readonly expected_valid_days?: unknown
...
/** One proposed summary slot (`user.*` / `history.*`). */
export interface ProposedSummary {
  readonly summary?: unknown
  readonly shouldUpdate?: unknown
  readonly scope?: unknown
  readonly authority?: unknown
}

/** One proposed contradiction removal (`factsToRemove[]`). */
export interface ProposedRemoval {
  readonly id?: unknown
  readonly scope?: unknown
  readonly reason?: unknown
  readonly replacementFactIndex?: unknown
}

/** One proposed consolidation group (`factsToConsolidate[]`). */
export interface ProposedConsolidation {
  readonly sourceIds?: unknown
  readonly consolidated?: ProposedFact
}

/** The extractor's whole output document. */
export interface ProposedUpdates {
  readonly user?: Record<string, ProposedSummary>
  readonly history?: Record<string, ProposedSummary>
  readonly newFacts?: unknown
  readonly factsToRemove?: unknown
  readonly factsToConsolidate?: unknown
}

/** The minimum a caller must tell the gate about a fact already on disk. */
...
export interface WriteGateOptions {
  readonly existingFacts?: readonly ExistingFact[]
  readonly confidenceThreshold?: number
  readonly maxFacts?: number
  readonly expectedValidDaysCeiling?: number
}

/** A fact that survived the gate, with its extraction-only labels stripped. */
export interface AcceptedFact {
  /** Index into the source array (`newFacts` or `factsToConsolidate`). */
  readonly index: number
  readonly origin: 'newFacts' | 'factsToConsolidate'
  readonly content: string
  readonly category: FactCategory
  readonly confidence: number
  readonly expectedValidDays?: number
  readonly sourceError?: string
  /** Only set for consolidation groups: the source fact ids to delete on success. */
  readonly consolidatedFrom?: readonly string[]
}

export interface AcceptedSummary {
  readonly section: 'user' | 'history'
...
export interface AcceptedRemoval {
  ...
  readonly replacementFactIndex?: number
}

/** Why one proposed item was refused. `reason` is a stable machine-readable code. */
export interface Rejection {
  readonly kind: 'fact' | 'summary' | 'removal' | 'consolidation'
...
export interface WriteGateResult {
  readonly acceptedFacts: readonly AcceptedFact[]
  readonly acceptedSummaries: readonly AcceptedSummary[]
  readonly acceptedRemovals: readonly AcceptedRemoval[]
  readonly rejections: readonly Rejection[]
  /** Existing fact ids evicted by the `max_facts` trim (lowest confidence first). */
  readonly trimmedExistingIds: readonly string[]
  /** Rejection count over total proposed items — upstream's `rejected_by_scope_gate` ratio. */
 ...
 */
export function coerceConfidence(value: unknown, fallback = MALFORMED_CONFIDENCE_DEFAULT): number {
 ...
 */
export function clampExpectedValidDays(value: unknown, ceiling = DEFAULT_EXPECTED_VALID_DAYS_CEILING): number | undefined {
  if (typeof value === 'boolean' || typeof value !== 'number' || !Number.isFinite(value)) return undefined
  const truncated = Math.trunc(value)
  if (truncated < 1) return undefined
...
function isLabelled(value: unknown, allowed: readonly string[]): boolean {
 ...
 */
function gateLabels(item: { scope?: unknown; durability?: unknown; authority?: unknown }): string | null {
  if (!isLabelled(item.scope, ['user', 'thread', 'project'])) return 'missing_or_invalid_scope'
  if (!isLabelled(item.durability, ['durable', 'temporary'])) return 'missing_or_invalid_durability'
  if (!isLabelled(item.authority, ['descriptive', 'transactional'])) return 'missing_or_invalid_authority'
  if (item.scope !== 'user') return 'scope_not_user'
  if (item.durability !== 'durable') return 'durability_not_durable'
  if (item.authority !== 'descriptive') return 'authority_not_descriptive'
  return null
}

interface Candidate {
  readonly accepted: AcceptedFact
  readonly key: string
}

function gateFact(
  raw: ProposedFact,
  origin: AcceptedFact['origin'],
  index: number,
  locator: string,
  threshold: number,
  ceiling: number,
  ...
  if (typeof raw !== 'object' || raw === null) return reject('not_an_object', 'proposed fact is not an object')
  if (typeof raw.content !== 'string' || raw.content.trim() === '') return reject('empty_content', 'fact.content must be a non-empty string')
  ...
  if (confidence < threshold) return reject('below_confidence_threshold', `confidence ${confidence} < ${threshold}`)
 ...
 */
export function applyWriteGate(proposal: ProposedUpdates, options: WriteGateOptions = {}): WriteGateResult {
  ...
  // ── 1. Summaries ─────────────────────────────────────────────────────────────────────
  const acceptedSummaries: AcceptedSummary[] = []
  for (const section of ['user', 'history'] as const) {
    ...
    for (const [slot, raw] of Object.entries(slots)) {
      ...
      if (raw.shouldUpdate !== true) continue
      proposedCount += 1
  ...
  newFacts.forEach((raw, index) => {
    proposedCount += 1
    const candidate = gateFact(raw as ProposedFact, 'newFacts', index, `newFacts[${index}]`, threshold, ceiling, rejections)
  ...
  groups.forEach((rawGroup, index) => {
    ...
    const group = rawGroup as ProposedConsolidation
    if (typeof group !== 'object' || group === null) {
      rejections.push({ kind: 'consolidation', locator, reason: 'not_an_object', detail: 'consolidation group is not an object' })
      return
    }
    const sourceIds = Array.isArray(group.sourceIds) ? group.sourceIds.filter((id): id is string => typeof id === 'string' && id !== '') : []
  ...
  const trimmedExistingIds: string[] = []
  const keptCandidates = new Set<Candidate>()
  for (const entry of kept) if (entry.slot.candidate !== undefined) keptCandidates.add(entry.slot.candidate)
  ...
  const acceptedRemovals: AcceptedRemoval[] = []
  ...
  removals.forEach((rawRemoval, index) => {
    ...
    const removal = rawRemoval as ProposedRemoval
