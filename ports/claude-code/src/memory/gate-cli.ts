...
import { applyWriteGate, type ExistingFact, type ProposedUpdates, type WriteGateOptions } from './write-gate.js'

/** Rejection rate above which upstream logs a warning (manager.py:681-741). */
export const REJECTION_RATE_WARNING_THRESHOLD = 0.6

interface GateCliInput {
  proposal?: unknown
...
function coerceExistingFacts(value: unknown): ExistingFact[] | null {
...
let input: GateCliInput
try {
  input = JSON.parse(raw) as GateCliInput
} catch (error) {
...
const result = applyWriteGate(input.proposal as ProposedUpdates, { ...(input.options ?? {}), existingFacts })
