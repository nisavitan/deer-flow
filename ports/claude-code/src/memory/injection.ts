...
import { HISTORY_SLOTS, USER_SLOTS, type MemoryFact } from './store.js'
...
export const DEFAULT_GUARANTEED_CATEGORIES = ['correction'] as const
...
export function estimateTokens(text: string): number {
  ...
  for (const character of text) {
    ...
    if ((code >= 0x4e00 && code <= 0x9fff) || (code >= 0x3040 && code <= 0x30ff) || (code >= 0xac00 && code <= 0xd7a3)) cjk += 1
  }
 ...
 */
export function escapeForMemoryBlock(value: string): string {
  return value.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
}

function coerceConfidence(value: unknown): number {
  if (typeof value === 'boolean' || typeof value !== 'number' || !Number.isFinite(value)) return 0
 ...
 */
export function formatFactLine(fact: Partial<MemoryFact>): string | null {
  if (typeof fact.content !== 'string') return null
 ...
 */
function selectFactLines(ranked: readonly Partial<MemoryFact>[], tokenBudget: number): { lines: string[]; consumed: number } {
  const lines: string[] = []
  let consumed = 0
...
/** One summary slot as the injector consumes it. */
export interface InjectionSummarySlot {
  readonly summary?: string
}

/** Everything the injector renders: the six summary slots plus the selected facts. */
export interface InjectionMemoryData {
  readonly user?: Partial<Record<(typeof USER_SLOTS)[number], InjectionSummarySlot>>
  readonly history?: Partial<Record<(typeof HISTORY_SLOTS)[number], InjectionSummarySlot>>
  readonly facts?: readonly Partial<MemoryFact>[]
}

export interface InjectionOptions {
  readonly maxTokens?: number
  readonly guaranteedCategories?: readonly string[]
  readonly guaranteedTokenBudget?: number
}

const USER_LABELS: Record<(typeof USER_SLOTS)[number], string> = {
  workContext: 'Work',
  personalContext: 'Personal',
  topOfMind: 'Current Focus',
}

const HISTORY_LABELS: Record<(typeof HISTORY_SLOTS)[number], string> = {
  recentMonths: 'Recent',
 ...
 */
export function formatMemoryForInjection(data: InjectionMemoryData | null | undefined, options: InjectionOptions = {}): string {
  ...
  for (const slot of USER_SLOTS) {
    ...
    if (typeof summary === 'string' && summary !== '') userLines.push(`${USER_LABELS[slot]}: ${escapeForMemoryBlock(summary)}`)
  }
  ...
  for (const slot of HISTORY_SLOTS) {
    ...
    if (typeof summary === 'string' && summary !== '') historyLines.push(`${HISTORY_LABELS[slot]}: ${escapeForMemoryBlock(summary)}`)
  }
  ...
  if (facts.length > 0) {
    ...
    const matchesGuaranteed = (fact: Partial<MemoryFact>): boolean => {
      if (guaranteedCategories.size === 0) return false
    ...
    const byConfidenceDesc = (left: Partial<MemoryFact>, right: Partial<MemoryFact>): number => coerceConfidence(right.confidence) - coerceConfidence(left.confidence)
    ...
    if (regular.length > 0) {
      ...
      if (regularBudget > 0) regularLines = selectFactLines(regular, regularBudget).lines
    }

    allFactLines = [...guaranteedLines, ...regularLines]
    if (allFactLines.length > 0) sections.push(FACTS_HEADER + allFactLines.join('\n'))
  }
 ...
 */
export function buildMemoryBlock(data: InjectionMemoryData | null | undefined, options: InjectionOptions = {}): string {
