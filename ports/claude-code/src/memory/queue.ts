 ...
 */
export function extractTurn(transcript: string): { user: string; assistant: string } | null {
  let user = ''
  let assistant = ''
  for (const line of transcript.split('\n')) {
    ...
    const message = typeof entry['message'] === 'object' && entry['message'] !== null ? (entry['message'] as Record<string, unknown>) : entry
    const role = typeof message['role'] === 'string' ? message['role'] : typeof entry['type'] === 'string' ? (entry['type'] as string) : ''
...
export function queuePath(env?: NodeJS.ProcessEnv): string {
  return join(memoryRoot(env), QUEUE_FILE_NAME)
}

/** Append one entry, creating the memory root on demand. */
export function appendQueueEntry(entry: QueueEntry, env?: NodeJS.ProcessEnv): void {
...
export function readQueue(env?: NodeJS.ProcessEnv): QueueEntry[] {
  let raw: string
  try {
  ...
  for (const line of raw.split('\n')) {
    ...
    try {
      ...
      if (typeof parsed['user'] !== 'string' || typeof parsed['assistant'] !== 'string') continue
      entries.push({
...
export function clearQueue(env?: NodeJS.ProcessEnv): void {
  try {
