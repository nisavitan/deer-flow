...
export const SALIENT_ARG_FIELDS = ['path', 'url', 'query', 'command', 'pattern', 'glob', 'cmd'] as const

/** Verbatim `_WARNING_MSG`. Model-facing text — do not reword. */
export const LOOP_WARNING_MESSAGE =
...
export const LOOP_HARD_STOP_MESSAGE =
...
export function toolFrequencyWarningMessage(toolName: string, count: number): string {
...
export function toolFrequencyHardStopMessage(toolName: string, count: number): string {
...
/** One tool call as the detector sees it: the original's `{"name": ..., "args": ...}` shape. */
export interface LoopToolCall {
  readonly name?: unknown
  /** dict, JSON string (some providers), or anything else — normalized defensively. */
...
export interface LoopDetectionConfig {
  ...
  /** Per-tool `[warn, hardLimit]`. Ships empty in the original. */
  readonly toolFreqOverrides: Readonly<Record<string, readonly [number, number]>>
}
 ...
 */
export function toolFrequencyWindowSize(config: LoopDetectionConfig): number {
  let size = Math.max(config.windowSize, config.toolFreqHardLimit)
  for (const override of Object.values(config.toolFreqOverrides)) {
    if (override[1] > size) size = override[1]
  }
  return size
}

/**
 * Serializable detector state — the port's stand-in for the original's four per-thread maps
 * (`_history`, `_warned`, `_tool_name_history`, `_tool_freq_warned`). Plain arrays so the whole
 * state round-trips through the state file with no reviver.
 */
export interface LoopDetectionState {
  /** `_history[thread]` — recent call-set hashes, oldest first, capped at `windowSize`. */
  readonly hashWindow: readonly string[]
  /** `_warned[thread]` — hashes already warned about, pruned as they decay out of the window. */
  readonly warnedHashes: readonly string[]
  /** `_tool_name_history[thread]` — recent tool names, oldest first, capped at the freq window. */
  readonly toolNameWindow: readonly string[]
  /** `_tool_freq_warned[thread]` — tool names already warned about at the frequency layer. */
  readonly toolFreqWarned: readonly string[]
}

/** A thread the detector has never seen. */
export const EMPTY_LOOP_DETECTION_STATE: LoopDetectionState = {
  hashWindow: [],
  warnedHashes: [],
  toolNameWindow: [],
  toolFreqWarned: [],
...
export type LoopDecision = 'none' | 'warn' | 'hard_stop'

export interface LoopStepResult {
  /** The state after this step. Callers persist this, never the input. */
  readonly state: LoopDetectionState
  readonly decision: LoopDecision
  /** Verbatim injected text for `warn` / `hard_stop`; `null` for `none`. */
  readonly message: string | null
  /** `"loop_capped"` on a hard stop, else `null` — the original's `consume_stop_reason` value. */
  readonly stopReason: typeof LOOP_STOP_REASON | null
  /** The multiset hash of this step's call set; `null` when there were no tool calls. */
  readonly callHash: string | null
  /** `history.count(call_hash)` after this step's append. */
  readonly hashCount: number
  /** The windowed per-tool-type counts after this step (the original's mirrored `Counter`). */
  readonly toolFrequencyCounts: Readonly<Record<string, number>>
}
 ...
 */
export function normalizeToolCallArgs(raw: unknown): [Record<string, unknown>, string | null] {
...
function pythonTruthyString(value: unknown): string {
 ...
 */
export function stableToolKey(name: string, args: Record<string, unknown>, fallbackKey: string | null): string {
  ...
  if (hasStable) return pythonJsonDumps(stableArgs)
 ...
 */
export function hashToolCalls(toolCalls: readonly LoopToolCall[]): string {
...
function toCountsRecord(counts: ReadonlyMap<string, number>): Record<string, number> {
  const record: Record<string, number> = {}
  for (const [key, value] of counts) record[key] = value
  return record
}
...
export function step(
  ...
  toolCalls: readonly LoopToolCall[],
  config: LoopDetectionConfig = DEFAULT_LOOP_DETECTION_CONFIG,
): LoopStepResult {
  const unchanged = (): LoopStepResult => ({
    state,
    decision: 'none',
    message: null,
    stopReason: null,
  ...
  if (toolCalls.length === 0) return unchanged()
  ...
  if (hashWindow.length > config.windowSize) hashWindow = hashWindow.slice(-config.windowSize)
  ...
  const settle = (
    ...
    message: string | null,
    extra: { warned?: readonly string[]; freqWarned?: ReadonlySet<string> } = {},
  ): LoopStepResult => ({
    state: {
      hashWindow,
  ...
  for (const call of toolCalls) {
    ...
    while (toolNameWindow.length > freqWindow) {
      const evicted = toolNameWindow.shift() as string
      const remaining = (toolNameCounts.get(evicted) ?? 0) - 1
      if (remaining <= 0) toolNameCounts.delete(evicted)
      else toolNameCounts.set(evicted, remaining)
    }
 ...
 */
export function parseLoopDetectionState(value: unknown): LoopDetectionState {
  if (typeof value !== 'object' || value === null) return EMPTY_LOOP_DETECTION_STATE
