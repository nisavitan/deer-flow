 ...
 */
export const SUBAGENT_STOP_REASON_VALUES = ['token_capped', 'turn_capped', 'loop_capped'] as const

export type SubagentStopReason = (typeof SUBAGENT_STOP_REASON_VALUES)[number]
 ...
 */
export const STOP_REASON_LABELS: Readonly<Record<SubagentStopReason, string>> = {
 ...
 */
export const RESULT_BEARING_STATUSES: ReadonlySet<SubagentStatus> = new Set<SubagentStatus>(['completed'])
 ...
 */
export const LEGACY_STATUS_NORMALIZATION: Readonly<Record<string, SubagentStopReason>> = {
...
export function isSubagentStatus(value: unknown): value is SubagentStatus {
  return typeof value === 'string' && (SUBAGENT_STATUS_VALUES as readonly string[]).includes(value)
}

export function isSubagentStopReason(value: unknown): value is SubagentStopReason {
  return typeof value === 'string' && (SUBAGENT_STOP_REASON_VALUES as readonly string[]).includes(value)
}
...
export class SubagentContractValueError extends Error {
  override readonly name = 'SubagentContractValueError'
  constructor(message: string) {
    super(message)
  }
}

export function assertSubagentStatus(value: unknown): SubagentStatus {
  if (!isSubagentStatus(value)) {
    throw new SubagentContractValueError(
      `invalid subagent status ${JSON.stringify(value)}; expected one of ${SUBAGENT_STATUS_VALUES.join(', ')}`,
    )
  }
  return value
}

export function assertSubagentStopReason(value: unknown): SubagentStopReason | null {
  if (value === null || value === undefined) return null
  if (!isSubagentStopReason(value)) {
    throw new SubagentContractValueError(
      `invalid subagent stop_reason ${JSON.stringify(value)}; expected one of ${SUBAGENT_STOP_REASON_VALUES.join(', ')}`,
    )
  }
  return value
}
...
export function stopReasonLabel(stopReason: SubagentStopReason | null | undefined): string | null {
 ...
 */
export function statusGuidance(status: string, stopReason?: SubagentStopReason | string | null): string {
  ...
  if (status === IN_PROGRESS_STATUS) return 'already delegated; do NOT delegate again; wait for or build on the result'
  if (status === 'completed') return 'completed result; do NOT delegate again; reuse this result'
  if (status === 'failed') return 'failed attempt; may retry with a changed plan'
  if (status === 'cancelled') return 'cancelled attempt; may retry with a changed plan'
  if (status === 'timed_out') return 'timed-out attempt; may retry with a changed plan'
  if (status === 'polling_timed_out') return 'polling timed-out attempt; may retry with a changed plan'
