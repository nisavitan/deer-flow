 ...
 */
export const SECRET_NAME_PATTERNS: readonly string[] = Object.freeze([
  '*KEY*',
  '*SECRET*',
  '*TOKEN*',
  '*PASS*',
 ...
 */
export const EXACT_DENYLIST: ReadonlySet<string> = Object.freeze(
  new Set([
    'DATABASE_URL',
    'DATABASE_URI',
    'REDIS_URL',
 ...
 */
export const BENIGN_PRESERVED: readonly string[] = Object.freeze([
  'PATH',
  'HOME',
  'SHELL',
  'LANG',
...
const COMPILED_SECRET_NAME_PATTERNS: readonly RegExp[] = SECRET_NAME_PATTERNS.map(fnmatchToRegExp)

/** True when `name` matches one of the wildcard patterns (the denylist is NOT consulted). */
export function matchesSecretNamePattern(name: string): boolean {
...
export function isSecretLikeName(name: string): boolean {
  const upper = name.toUpperCase()
  if (EXACT_DENYLIST.has(upper)) return true
  return COMPILED_SECRET_NAME_PATTERNS.some((pattern) => pattern.test(upper))
}
 ...
 */
export function detectSecretExposure(command: string, env: Readonly<Record<string, string | undefined>>): string[] {
  if (!command) return []
  const hits: string[] = []
  for (const [name, value] of Object.entries(env)) {
    if (value === undefined || value.length < MIN_EXPOSURE_VALUE_LENGTH) continue
    if (!isSecretLikeName(name)) continue
    if (command.includes(value)) hits.push(name)
  }
  return hits.sort()
}
