 ...
 * generation failures.
 */
export const CANNED_SUMMARIES: ReadonlySet<string> = new Set([
...
export interface SummaryInputOptions {
  /**
   * Total character budget shared by both blocks. Omit for no trimming — the original's
   * `trim_tokens_to_summarize=None` branch [summarization_middleware.py:379-382].
   */
 ...
 */
export function buildSummaryInputText(
  newMessages: string,
  existingSummary: string | null | undefined,
  options: SummaryInputOptions = {},
): string | null {
  const budget = options.charBudget
  let trimmedPreviousSummary: string
  let trimmedNewMessages: string
...
export interface SummaryRequestOptions extends SummaryInputOptions {
  /** Base instruction containing a `{messages}` placeholder. Defaults to the port-authored text. */
  readonly baseInstruction?: string
}

/**
 * The DeerFlow summary request for one compaction.
 *
 * `prompt` is `null` when the caller should short-circuit: `canned` then carries the valid
 * non-generated summary to commit, mirroring `_prepare_summary_prompt`
 * [summarization_middleware.py:222-233].
 */
export interface SummaryRequest {
  readonly prompt: string | null
  readonly canned: string | null
}
