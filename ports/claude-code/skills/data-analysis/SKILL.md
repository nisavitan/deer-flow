...
<!-- deerflow-origin: ported from skills/public/data-analysis/ @ 0950924
     port: script + upload/output/workspace paths; present_files -> final-message listing.
-->
...
- **File location**: Path(s) to the Excel/CSV files provided by the user
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/data-analysis/scripts/analyze.py \
  --files ./data.xlsx \
  --action inspect
```
...
python ${CLAUDE_PLUGIN_ROOT}/skills/data-analysis/scripts/analyze.py \
  ...
  --output-file outputs/filtered-results.csv
```
...
python ${CLAUDE_PLUGIN_ROOT}/skills/data-analysis/scripts/analyze.py \
  ...
  --output-file outputs/monthly-trends.csv
```
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/data-analysis/scripts/analyze.py \
  --files ./orders.csv ./customers.xlsx \
  --action query \
...
- For large results, export to a file under `outputs/` and list its path in your final message
...
- On first load, files are parsed and stored in a persistent DuckDB database under `.data-analysis-cache/` in the system temp directory (the location `scripts/analyze.py` actually uses)
