...
# Install a skill and link it to the project's .claude/skills directory
# (DeerFlow port: path constants adapted from skills/custom + deer-flow.code-workspace
#  to the Claude Code project skills directory. Logic unchanged.)
...
# Find project root by looking for .claude/ or .git/ (Claude Code project markers)
find_project_root() {
  local dir="$PWD"
  while [[ "$dir" != "/" ]]; do
...
PROJECT_ROOT="${CLAUDE_PROJECT_DIR:-$(find_project_root)}"

if [[ -z "$PROJECT_ROOT" ]]; then
  echo "Error: Could not find project root (no .claude/ or .git/ directory found)"
  exit 1
fi

SKILL_SOURCE="$HOME/.agents/skills/$SKILL_NAME"
SKILL_TARGET="$PROJECT_ROOT/.claude/skills"
