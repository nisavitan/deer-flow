...
<!-- deerflow-origin: ported from skills/public/systematic-literature-review/ @ 0950924
     port: script + outputs paths; present_files -> final-message listing (evals.json assertions updated to match).
-->
...
- **Output location**: where to save the final report (default `outputs/`).
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/systematic-literature-review/scripts/arxiv_search.py \
  "<topic>" \
  --max-results <N> \
  [--category <cat>] \
  [--sort-by relevance] \
...
Save the full report to `outputs/slr-<topic-slug>-<YYYYMMDD>.md` where `<topic-slug>` is a lowercased hyphenated version of the topic (e.g. `transformer-attention`). Then list that path in your final message so the user can open it.
...
5. Phase 5: save to `slr-transformer-attention-20260409.md`, list the report path in the final message.
